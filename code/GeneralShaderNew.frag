...
calculateLighting (Light light, vec3 vertexPosition, vec3 vertexNormal)
{
  ...
  vec3 lightEyePosition = mat3 (uView) * light.position;
  vec3 lightEyeDirection = transpose ( inverse(mat3 (uView))) * light.direction;
  ...
  { // Directional
    ...
    lightVector = normalize (-lightEyePosition);
  }
  else
  { // Point or spot
    //restore
    lightVector = normalize (lightEyePosition - vertexPosition);
        //change attempt
  ...
  if (lambertianCoef > 0.0)
  {
    ...
    { // Non-directional, so light attenuates
      float distance = length (vertexPosition - lightEyePosition);
  ...
  normalTransform = transpose (inverse (normalTransform));
      ...
      //restore value
      float cosTheta = dot (-lightVector, lightEyeDirection);
       //change
       //float cosTheta = dot (-lightVector, lightEyeDirection);
